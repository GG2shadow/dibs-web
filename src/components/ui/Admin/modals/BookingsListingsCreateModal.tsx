...
interface BookingsListingsCreateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate?: (newListing: BookingListing) => void;
}
...
export function BookingsListingsCreateModal({
  ...
  const [formData, setFormData] = useState<BookingListing>({
    title: '',
    shortDescription: '',
    longDescription: '',
    price: '',
            ...
            <form className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <Label htmlFor="requiresDeposit" className="font-['Inter']">Requires Deposit</Label>
                      <p className="text-sm text-muted-foreground">Enable if this listing requires a deposit</p>
                    </div>
                    <Switch
                      id="requiresDeposit"
                      checked={formData.requiresDeposit}
                      onCheckedChange={() => handleToggleChange('requiresDeposit')}
                    />
                  </div>
                </div>
              </div>
